class Triage::CreateIssueResolutionProcessTicketJob < ApplicationJob
  def perform(issue, triage_group:, triage_owner_id:, triage_zammad_client: TriageZammadEnvironment.client)
    resolution_external_ticket = triage_zammad_client.create_ticket_from_issue!(
      issue,
    ...
    issue.update!(
      resolution_external_id: resolution_external_ticket.id,
      last_synced_at: Time.now
    )

    triage_zammad_client.link_tickets!(
      parent_ticket_id: issue.triage_external_id,
      child_ticket_id: resolution_external_ticket.id
    )

    triage_zammad_client.create_system_note!(
      issue.triage_external_id,
      "Triáž podnetu bola ukončená a bol vytvorený nový tiket <a href=\"/#ticket/zoom/#{resolution_external_ticket.id}\">##{resolution_external_ticket.number}</a> na jeho vyriešenie.",

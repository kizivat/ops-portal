class Connector::UpdateTriageIssueFromBackofficeJob < ApplicationJob
  def perform(tenant, ticket_id, zammad_client: Connector::BackofficeZammadEnvironment.client(tenant), ops_api_client: Connector::OpsApiClient)
    issue_data = zammad_client.get_issue(ticket_id)

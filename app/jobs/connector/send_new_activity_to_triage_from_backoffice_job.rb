class Connector::SendNewActivityToTriageFromBackofficeJob < ApplicationJob
  def perform(tenant, ticket_id, article_id, zammad_client: Connector::BackofficeZammadEnvironment.client(tenant), ops_api_client: Connector::OpsApiClient)
    return if tenant.activities.find_by(backoffice_external_id: article_id)&.triage_external_id

    activity = zammad_client.get_activity(ticket_id, article_id)

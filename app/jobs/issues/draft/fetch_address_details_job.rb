class Issues::Draft::FetchAddressDetailsJob < ApplicationJob
  ...
  def perform(draft)
    ...
    draft.address_road = address.find { _1["class"] == "highway" }&.dig("localname")
    draft.address_city_district = address.find { _1["type"] == "administrative" && _1["admin_level"] == 9 }&.dig("localname")
    ...
    draft.address_county = address.find { _1["type"] == "administrative" && _1["admin_level"] == 4 }&.dig("localname")
